> 功能已于 2026-07-07 决策移除，本文档仅作历史参考。

# Pinned Action / Live Runner 专项
...
本专项只讨论 Pinned Action / Live Runner：把高频 Action 固定成常驻工具，支持私有输入区、实时输出区、动态控制面板、懒加载、超时释放和 Tombstone 恢复。

“扫描现有脚本、把可推断 mode 写进脚本”的优化单独走 `builtin-script-auto-inference.md`，不要混在本专项里推进。
...
```text
Pinned Action 的主流程应该是：粘贴输入 -> 看到输出。
控制面板只用于补充无法从输入或上下文确定的用户意图。
```
...
这些参数应通过之前设计的 Panel 能力承载，而不是塞进主输入区，也不要变成每次运行前的阻塞步骤。
...
Pinned Action 依赖 Action 自身的行为。如果某个脚本已经能自适应输入，Pinned Runner 会自然受益：用户粘贴输入后，右侧输出更少需要控制面板。

但两件事分开推进：

```text
builtin-script-auto-inference.md:
  优化内置脚本自身的 auto 判断。

pinned-action-live-runner.md:
  设计 pin、sidebar、live runner、controls panel、懒加载、tombstone。
```

Pinned Action 不负责替脚本猜业务语义。它只负责运行、展示、控制、释放和恢复。
...
### Milestone A：Pinned Action 数据模型与 Sidebar
...
已支持自适应的脚本不应强制先打开 controls。
关闭 controls 不影响 runner。
```

### Milestone F：全局 Launcher
...
```text
改了哪些 Pinned Action 能力
新增或调整了哪些状态/API
动态加载、释放、Tombstone 的验证结果
Controls Panel 的行为和冲突处理
真实 UI 验证结果
剩余风险
```
