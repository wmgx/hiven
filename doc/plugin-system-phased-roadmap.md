...
```text
manifest.json
index.js or index.tsx fixed entry
assets
README
```
...
manifest 再写一份容易不一致
运行入口固定为 index.tsx / index.ts / index.jsx / index.js / index.mjs 的第一项
安装/启用时以固定入口返回的 PluginDefinition 为准
```
...
用户选择本地插件目录
  -> 读取 manifest.json
  -> 校验 pluginId / version / fixed index.* entry
  -> 检查 pluginId 是否冲突
...
用户对本地插件目录开启 Watch
  -> 初次执行 Side-load
  -> 监听 manifest/index.* entry/assets 变化
  -> 变化后 dispose 该 dev plugin
  -> 重新 import fixed entry
  -> 重新注册到 dev registry
...
以下 spike 必须在正式实现 PluginRuntime 前完成。它们会决定插件包格式、fixed entry 要求、reload 策略和 renderer contribution 的真实类型。
...
将固定 index.* entry 转成前端可 import 的 URL
动态 import fixed entry
