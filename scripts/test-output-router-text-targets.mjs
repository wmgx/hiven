...
assert.match(outputRouter, /ctx\.openPluginSurface\(text, target\)/, 'routeTextOutput must provide text to open-plugin-surface outputs')
...
assert.match(outputRouter, /attachEditorPanel:[\s\S]*showPluginSurfaceWindow\(\{[\s\S]*pluginId:[\s\S]*surfaceId:/, 'attach-editor-panel target must send panel attach requests through editor bridge')
...
assert.match(outputRouter, /saveToShelf:[\s\S]*createQuickEditorPane\(\{[\s\S]*text/, 'save-to-shelf target must route through the editor bridge into the output shelf panel')
