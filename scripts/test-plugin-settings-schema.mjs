...
assert.match(files.pluginTypes, /unitI18n\??:\s*Partial<Record<Locale,\s*string>>/, 'number settings should support localized units')
assert.match(files.pluginTypes, /icon\??:\s*string/, 'settings fields should support host-owned icon hints')
assert.match(files.pluginTypes, /requires\??:\s*PluginPermission\[\]/, 'settings fields should declare permission dependencies')
assert.match(files.pluginTypes, /itemTitleKey\??:\s*string/, 'object-list fields should declare the item title source')
...
assert.match(schemaRenderer, /isRenderableField/, 'schema renderer must omit schema fields that do not expose a real choice')
assert.match(schemaRenderer, /field\.options\.length\s*>\s*1/, 'single-option select fields must not render as fake controls')
...
assert.match(files.webOpen, /key:\s*['"]enabled['"]/, 'web quick open schema should expose enabled switch')
assert.doesNotMatch(files.webOpen, /key:\s*['"]defaultOpenMode['"]/, 'web quick open must not expose a default-open-mode selector until there is more than one real open target')
assert.match(files.webOpen, /itemTitleKey:\s*['"]title['"]/, 'web quick open rule cards should title themselves from each rule name')
...
assert.doesNotMatch(files.webOpen, /key:\s*['"]openMode['"]/, 'web quick open rule cards must not expose open mode until a second open capability exists')
...
assert.match(files.clipboardHistory, /unitI18n:\s*\{\s*zh:\s*['"]天['"]\s*\}/, 'clipboard history day unit must be localized through unitI18n')
assert.match(files.clipboardHistory, /requires:\s*\[[\s\S]{0,120}clipboard\.watch/, 'clipboard history settings should declare permission dependencies')
assert.doesNotMatch(files.clipboardHistory, /key:\s*['"]defaultAction['"]|key:\s*['"]pasteFailureFallback['"]/, 'clipboard history must not expose fixed behavior as single-option settings')
