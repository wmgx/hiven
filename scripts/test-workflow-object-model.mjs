...
assert.match(files.workflowLauncherAdapter, /subtitle:\s*objectActionSubtitle\(object\)/, 'Workflow object launcher rows must show an explicit action-expansion hint')
assert.match(files.workflowLauncherAdapter, /function\s+objectActionSubtitle\(object:\s*WorkObject\):\s*string[\s\S]*Tab[\s\S]*actions/, 'Workflow object subtitles must teach the user to press Tab for actions')
...
assert.match(files.surfaceActions, /surface\.kind === ['"]settings['"][\s\S]*showLauncherWindow\(\)[\s\S]*openLauncherHostSurface\(['"]settings['"]\)/, 'Surface focus must route Settings surfaces through the launcher host surface')
assert.match(files.surfaceActions, /surface\.kind === ['"]plugins['"][\s\S]*showLauncherWindow\(\)[\s\S]*openLauncherHostSurface\(['"]plugins['"]\)/, 'Surface focus must route Plugins surfaces through the launcher host surface')
