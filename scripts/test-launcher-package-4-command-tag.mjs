...
assert.match(collect, /launcher-preview-well|LauncherEmptyWell/, 'preview well UI')
assert.match(collect, /LauncherOutputTargetsBar|useOutputDestinations/, 'destination badges via shared module')
...
assert.match(collect, /resolveFromKeyboard/, 'keyboard maps ⇧/⌘ enter via shared helper')

const dest = readFileSync('src/components/launcher/LauncherOutputTargets.tsx', 'utf8')
assert.match(dest, /paste-foreground/, 'paste destination')
assert.match(dest, /⇧↵/, 'paste uses Shift+Enter badge')
assert.match(dest, /return-to-launcher/, 'return-to-launcher destination')
