...
assert.match(clipboardImpl, /emitChange\(|onChange\(change\)/, 'Must call onChange callback on changes')
assert.match(clipboardImpl, /changeTail/, 'Watch must serialize async onChange handlers to avoid concurrent write races')
...
assert.match(pasteImpl, /hide_launcher_and_paste/, 'Must try Tauri hide_launcher_and_paste command')
