...
assert.match(files.clipboardStyle, /\.clipboard-history-item-row\.is-selected[\s\S]{0,260}var\(--hiven-color-accent\)/, 'clipboard-history selected item should use visible accent styling')
assert.match(files.clipboardStyle, /\.clipboard-history-main::after[\s\S]{0,260}left:\s*42%[\s\S]{0,180}width:\s*1px/, 'clipboard-history left and right panes should have a visible independent divider')
assert.match(files.clipboardStyle, /\.clipboard-history-item-row:hover[\s\S]{0,220}border-color[\s\S]{0,220}background/, 'clipboard-history items should keep a visible hover highlight')
