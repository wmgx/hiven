...
assert.match(commandAdapter, /runTextPluginCommand/, 'launcher command adapter should execute text commands through the shared runner')
assert.match(commandAdapter, /replaceActiveTextResult/, 'launcher command adapter should map text command output to replace-active-text primary action')
assert.match(commandAdapter, /output\.kind\s*===\s*['"]error['"][\s\S]*errorResult/, 'launcher command adapter should surface text command errors as launcher errors')
