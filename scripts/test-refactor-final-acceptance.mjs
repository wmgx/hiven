...
assert.equal(
  packageJson.scripts?.['test:workspace-public-api-window-boundary'],
  'node scripts/test-workspace-public-api-window-boundary.mjs',
  'package.json must expose workspace public API window-boundary coverage',
)
assert.equal(
  packageJson.scripts?.['test:effect-runner-window-boundary'],
...
assert.match(files.pluginInputResolver, /if \(!isEditorWindowRuntime\(\)\)[\s\S]*getActiveEditorContextSnapshot\(\)[\s\S]*resolveWithoutEditorContext/, 'plugin input resolver must use synced editor context outside the editor runtime and avoid local shadow workspace fallback')
...
assert.match(files.inputResolver, /if \(!isEditorWindowRuntime\(\)\)[\s\S]*getActiveEditorContextSnapshot\(\)[\s\S]*resolveMissingEditorContextInput/, 'generic input resolver must use synced editor context outside the editor runtime and avoid local shadow workspace fallback')
assert.match(files.workspacePublicApi, /function readEditorContextSnapshot\(\)[\s\S]*getActiveEditorContextSnapshot\(\)[\s\S]*function readEditorPaneSnapshot\(\)[\s\S]*getActiveEditorPaneSnapshot\(\)/, 'workspace public API must read synced editor context and pane snapshots outside editor windows')
...
assert.match(files.effectRunner, /runnableEffects[\s\S]*Editor workspace effects can only run in the editor window[\s\S]*for \(const effect of runnableEffects\)/, 'effect runner must reject editor workspace effects outside the editor window')
