...
assert.match(pluginIndex, /key:\s*['"]maxTextBytes['"][\s\S]{0,220}storageScale:\s*MB/, 'Text byte limit must render as MB via schema')
assert.match(pluginIndex, /key:\s*['"]maxImageBytes['"][\s\S]{0,220}storageScale:\s*MB/, 'Image byte limit must render as MB via schema')
assert.match(pluginIndex, /key:\s*['"]maxTotalCacheBytes['"][\s\S]{0,220}storageScale:\s*MB/, 'Total cache limit must render as MB via schema')
assert.match(pluginIndex, /key:\s*['"]defaultAction['"]/, 'Settings schema must expose default action')
assert.match(pluginIndex, /key:\s*['"]pasteFailureFallback['"]/, 'Settings schema must expose paste fallback')
