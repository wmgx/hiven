...
has(files.app, /updateSetting\(['"]theme['"]/, 'App should provide an in-app theme toggle')
has(files.app, /flux-titlebar/, 'App should render the spatial titlebar')
has(files.app, /onMouseDown=\{handleOpenLauncher\}/, 'Titlebar launcher button should work inside draggable titlebar regions')
...
has(files.css, /--titlebar-h:\s*32px/, 'CSS should expose demo-compatible titlebar height')
has(files.css, /--sidebar-w:\s*44px/, 'CSS should expose demo-compatible sidebar width')
has(files.css, /\.flux-titlebar\b/, 'CSS should style the spatial titlebar')
...
has(files.paneEditor, /lineDecorationsWidth:\s*8/, 'Primary editor should keep a consistent VS Code-like gap after compact line numbers')
