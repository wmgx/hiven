...
const files = {
  ...
  scripts: read('src/surfaces/PluginsManagerSurfaceContent.tsx'),
  settings: read('src/surfaces/SettingsSurfaceContent.tsx'),
...
has(files.css, /\.global-launcher-panel[\s\S]{0,260}max-height:\s*min\(var\(--launcher-panel-max-height\),\s*calc\(100vh - 24px\)\)/, 'Global launcher panel should be constrained within the viewport')
