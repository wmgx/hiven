...
has(files.app, /data-theme=\{theme\}/, 'Launcher app should apply persisted theme through data-theme')
has(files.app, /setTheme\(theme\)/, 'App should sync the native Tauri window theme with the persisted theme')
...
has(files.settings, /updateSetting\(['"]theme['"]/, 'Launcher-hosted settings should provide the in-app theme toggle')
has(files.settings, /settings\.theme === ['"]dark['"]/, 'Settings should derive the theme toggle from persisted settings')
...
has(files.css, /--sidebar-w:\s*48px/, 'CSS should expose demo-compatible sidebar width')
...
has(files.scripts, /scripts-tabs|ptabs/, 'Scripts view should use tab controls for plugin tabs')
...
has(files.commandPalette, /global-launcher-result-row|launcher-item-title/, 'Command palette should render launcher-style command rows')
has(files.globalLauncher, /global-launcher-result-row|launcher-item-title|l-row/, 'Global launcher should render launcher-style command rows')
...
hasTokenRule(files.monacoTheme, 'string', '9db0d8', 'Dark Monaco theme should align string highlighting with the hiven dark palette')
...
assert.equal(cssColor(files.monacoTheme, 'editorBracketHighlight.foreground2'), '#3b82f6', 'Dark Monaco bracket pair color 2 should align with the hiven accent')
...

{
  ...
  assert.equal(main, undefined, 'launcher-first Tauri config should not define a persistent main window')
