...
assert.match(globalLauncher, /previewChoice[\s\S]*frame\.previewOutput\?\.choices\[0\]/, 'GlobalLauncher should render controller preview output inside the manual input frame')
assert.match(globalLauncher, /previewChoice[\s\S]*palette\.enterToCopy/, 'GlobalLauncher should show Enter-to-copy wording when a live preview is available')
