...
assert.match(lineToolsPlugin, /id:\s*['"]reverse['"][\s\S]{0,360}ctx\.output\.replaceActiveText/, 'reverse launcher tool should replace active text, not copy-only')
assert.match(launcherRegistry, /adaptCommandToLauncherItem/, 'safe text commands should enter the launcher through the registry command adapter')
assert.match(launcherRegistry, /addCoveredCommandAliases/, 'launcher registry should dedupe command-adapter entries covered by tools/items')
assert.match(launcherRegistry, /coveredCommandIds\.has\(command\.id\)/, 'covered legacy commands should not be exposed as duplicate launcher items')
