...
assert.match(tauriLib, /async fn hide_plugin_surface_window[\s\S]*surface_registry_mark_record_state\(&label,\s*["']hidden["']/, 'native hide must persist hidden state in the Rust SurfaceRegistry')
assert.match(tauriLib, /emit_surface_registry_mark_state/, 'native plugin surface lifecycle must broadcast SurfaceRegistry state changes')
