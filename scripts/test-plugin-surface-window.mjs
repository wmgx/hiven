...
const windowManager = read('src/workspace/pluginSurfaceWindows.ts')
...
assert.match(windowComponent, /requestHidePluginSurfaceWindow\(target\)/, 'Escape must route hiding through the plugin surface window manager')
assert.match(windowManager, /invoke\(['"]hide_plugin_surface_window['"]/, 'frontend window manager must call the native hide command')
assert.match(windowManager, /markSurfaceInstanceState\(pluginSurfaceInstanceId\(target\),\s*['"]hidden['"]\)/, 'hiding a plugin surface window must mark the surface hidden')
...
assert.match(windowManager, /destroyTimeoutMs/, 'frontend window manager must pass destroyTimeout to native runtime')
