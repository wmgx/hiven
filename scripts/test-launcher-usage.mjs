...
assert.equal(u1['command-palette']['plugin:p:launcher:a'].count, 1)
assert.equal(u1['command-palette']['plugin:p:launcher:a'].lastSelectedAt, 1000)
...
// immutability
assert.equal(Object.keys(u0['command-palette']).length, 0, 'input not mutated')

const u2 = mod.recordSelection(u1, 'command-palette', 'plugin:p:launcher:a', 2000)
assert.equal(u2['command-palette']['plugin:p:launcher:a'].count, 2, 'count increments')
assert.equal(u2['command-palette']['plugin:p:launcher:a'].lastSelectedAt, 2000)
...
assert.equal(us['command-palette']['plugin:p:launcher:a'].count, 2)
...
assert.equal(migrated['command-palette']['plugin:demo:launcher:line-tools.reverse'].count, 5, 'cp count preserved')
assert.equal(migrated['command-palette']['plugin:demo:launcher:base64.run'].count, 2)
...
const rev = migrated['command-palette']['plugin:demo:launcher:line-tools.reverse'].lastSelectedAt
const b64 = migrated['command-palette']['plugin:demo:launcher:base64.run'].lastSelectedAt
...
assert.equal(Object.keys(dropped['command-palette']).length, 0, 'unmapped keys dropped')
