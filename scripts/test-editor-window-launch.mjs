...
assert.match(main, /windowType === ['"]quick-editor['"][\s\S]*QuickEditorDetachedView/, 'main entry must render EditorWindow for ?window=editor')
assert.match(editorWindowApi, /closeQuickEditorWindow\(\)/, 'frontend editor closer must delegate lifecycle to native runtime')
assert.match(editorWindowApi, /closeQuickEditorWindow\(\)/, 'frontend editor closer must delegate lifecycle to native runtime')
assert.match(editorWindowApi, /markSurfaceInstanceState\(.*QUICK_EDITOR_WINDOW_LABEL,\s*['"]destroyed['"]\)/, 'frontend editor closer must mark editor surface hidden using the centralized label')
...
assert.match(tauriLib, /async fn show_quick_editor_window/, 'native runtime must expose show_editor_window')
assert.match(tauriLib, /async fn close_quick_editor_window/, 'native runtime must expose close_editor_window')
assert.match(tauriLib, /close_quick_editor_window,[\s\S]*show_plugin_surface_window/, 'close_editor_window must be registered with Tauri')
...
assert.match(tauriLib, /WebviewUrl::App\("index\.html\?window=quick-editor"/, 'editor window must route to ?window=editor')
...
assert.match(tauriLib, /show_quick_editor_window[\s\S]*set_focus/, 'show_editor_window must focus the editor window')
...
assert.match(editorWindow, /return \(\) => \{[\s\S]*clearActiveEditorSnapshots\(\)[\s\S]*markSurfaceInstanceState\(windowLabel,\s*['"]hidden['"]\)/, 'EditorWindow must clear active editor snapshots on unmount before marking hidden')
...
assert.match(editorWindow, /upsertSurfaceInstance\([\s\S]*id:\s*windowLabel/, 'EditorWindow must register itself as a surface using the centralized label')
