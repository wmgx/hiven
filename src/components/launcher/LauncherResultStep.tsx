...
export function LauncherResultStep({ frame, error, busy, onActivateChoice, onActivateSecondary, onSubmitSelection, onBack, locale }: {
  frame: ResultFrame
  error: string | null
  busy: boolean
  ...
  return (
    <>
      ...
      <div className="global-launcher-footer l-foot">
        ...
        {selection && <LauncherHintKey keys="space" label={t(locale, 'palette.toggle')} />}
        <LauncherHintKey keys="↵" label={selection?.submitTitle ?? t(locale, 'palette.select')} />
        {selection && (
